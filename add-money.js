...
function showQR() {
    ...
    if(amt < 3 || amt === "") return alert("Minimum amount is ₹3");

    // --- QR Update Logic Start ---
    const upiID = "8891611790@fam"; // Ningalude UPI ID
    const name = "KL ESPORTS"; // App Name
    
    // UPI Link with Amount
    const upiLink = `upi://pay?pa=${upiID}&pn=${encodeURIComponent(name)}&am=${amt}&cu=INR`;
    
    // QR Server API link generate cheyyunnu
    const qrApiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(upiLink)}`;
    
    // HTML-le img tag-le src maattunnu
    document.getElementById('myQr').src = qrApiUrl;
    // --- QR Update Logic End ---
